...
static int print_usage()
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr, "-l BC7: Use linear colorspace metrics instead of perceptual (the default is perceptual or sRGB!)\n");
	...
	fprintf(stderr, "RDO optimizations:\n");
	fprintf(stderr, "-z# BC1: Set RDO quality, lower=higher quality/larger LZ compressed files, ranges from (0,8], try .6-4\n");
	...
	fprintf(stderr, "-e BC7: Quantize/weight BC7 output for lower entropy\n");
	fprintf(stderr, "-E BC7: Entropy reduction transform, use -z option to specify lambda, use with -e\n");
	fprintf(stderr, "\n");
...
int main(int argc, char *argv[])
{
	...
	uint32_t m_lookback_window_size = 256;//rgbcx::BC1_RDO_DEFAULT_LZ_DICT_SIZE;
	...
	bool bc7_reduce_entropy_transform = false;
	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (pArg[0] == '-')
		{
			switch (pArg[1])
			{
				...
				case 'E':
				{
					bc7_reduce_entropy_transform = true;
					break;
				}
				case 'h':
				{
...
#pragma omp critical
	...
	if (rdo_q > 0.0f)
	{
		int rdo_total_threads = std::min(max_threads, 20);
		if (total_blocks <= 16384)
			rdo_total_threads = std::min(rdo_total_threads, 5);
		else if (total_blocks <= 32768)
			rdo_total_threads = std::min(rdo_total_threads, 10);
		...
		if ((dxgi_format == DXGI_FORMAT_BC7_UNORM) && (bc7_reduce_entropy_transform))
		{
...
#pragma omp parallel for
				for (int p = 0; p < rdo_total_threads; p++)
				{
					...
					bool status = bc7enc_reduce_entropy(&packed_image16[first_block_to_encode], blocks_to_encode, (color_rgba*)&block_pixels[16 * first_block_to_encode], bc7_rdo_p, bc7_pack_params, total_modified_local);
...
#pragma omp critical
			...
			{
				bool status = bc7enc_reduce_entropy(&packed_image16[0], total_blocks, (color_rgba*)&block_pixels[0], bc7_rdo_p, bc7_pack_params, total_modified);
				if (!status)
				{
					fprintf(stderr, "bc7enc_reduce_entropy failed!\n");
					exit(EXIT_FAILURE);
		...
		else if (dxgi_format == DXGI_FORMAT_BC7_UNORM)
		{
			printf("\nBC7 RDO:\n");
			...
			if (selector_rdo)
			{
				...
				uint32_t total_skipped = 0, total_endpoints_refined = 0, total_merged = 0, total_smooth = 0;
...
#pragma omp parallel for
					for (int p = 0; p < rdo_total_threads; p++)
					{
						...
						uint32_t total_skipped_local = 0, total_endpoints_refined_local = 0, total_merged_local = 0, total_smooth_local = 0;
						bool status = bc7enc_rdo_postprocess(&packed_image16[first_block_to_encode], blocks_to_encode, (color_rgba*)&block_pixels[16 * first_block_to_encode], bc7_rdo_p,
							total_skipped_local, total_endpoints_refined_local, total_merged_local, total_smooth_local,
							true, false,
							bc7_pack_params);

#pragma omp critical
						{
							total_skipped += total_skipped_local;
							total_endpoints_refined += total_endpoints_refined_local;
							total_merged += total_merged_local;
							total_smooth += total_smooth_local;
						}

						if (!status)
						{
							fprintf(stderr, "bc7enc_rdo_postprocess failed!\n");
							exit(EXIT_FAILURE);
						}
					} // p
				}
				...
				{
					bool status = bc7enc_rdo_postprocess(&packed_image16[0], total_blocks, (color_rgba*)&block_pixels[0], bc7_rdo_p, total_skipped, total_endpoints_refined, total_merged, total_smooth,
						true, false,
						bc7_pack_params);
				...
				printf("Total selector RDO postprocess time: %f secs\n", (double)(rdo_end_t - rdo_start_t) / CLOCKS_PER_SEC);

				printf("Total blocks skipped: %u %3.2f%%, Total blocks merged: %u %3.2f%%, Total block endpoints refined: %u %3.2f%%, Total smooth blocks: %u %3.2f%%\n",
					...
					total_smooth, total_smooth * 100.0f / total_blocks);
			}

			if ((selector_rdo) && (has_alpha))
			{
				printf("--- Begin selector RDO post-process mode 5 alpha:\n");
...
#pragma omp critical
				...
				printf("Total endpoint RDO postprocess time: %f secs\n", (double)(rdo_end_t - rdo_start_t) / CLOCKS_PER_SEC);
